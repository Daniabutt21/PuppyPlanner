"""
This module contains Web UI tests for the PuppyPlanner app.
"""
...
def test_successful_login(page: Page, user: User):
  ...
  expect(page).to_have_title('Reminders | PuppyPlanner reminders app')
  expect(page).to_have_url(re.compile(re.escape('/') + 'reminders'))
  expect(page.locator('id=pupplyplanner-logo')).to_be_visible()
  expect(page.locator('id=pupplyplanner-title')).to_have_text('PuppyPlanner')
